...
router.get("/members/region/:idRegion",(req,res)=>{
        ...
        else if(!document) res.status(404).send(`region with id ${req.params.idRegion} not found`)
        else {
            Member.find({_regionId : req.params.idRegion, state : 'Accepted'}).sort({inscriptionDate : -1}).then(result=>{
               if (!result) {res.status(404).send(`no member for region with id ${document._id}`)}
                else if(result) {
                    res.status(200).json({
